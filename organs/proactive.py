...
class ProactiveGreeter:
    def __init__(self,ap,_load_config,host,waifu_cache):
        self.ap = ap
        self.host = host
        self._generator = Generator(ap)
        self._load_config = _load_config
        self.target_qq = None
        self.waifu_cache = waifu_cache
    ...
    async def start_loop(self):
        try:
            ...
            while True:
                ...
                if self.waifu_cache:
                    self.current_config = self.waifu_cache.get(self.target_qq)
                    if not self.target_qq or not self.current_config:
                        self.ap.logger.info(f"target_qq or current_config is invalid {self.target_qq}")
                        await asyncio.sleep(initial_loop_delay)
                        self.ap.logger.info(
                            f"sleep for {initial_loop_delay}seconds and try again search target_qq or current_config")
                        count += 1
                    else:
                    ...
                    count+=1
                if count == 10:
                    self.ap.logger.waring(f"fail to find waifu_cache")
                    break
    ...
    async def _proactive_loop(self):
        ...
        if self.current_config.proactive_greeting_enabled and self.current_config.summarization_mode:
            self.ap.logger.info(
            ...
            try:
                ...
                return
            while True:
                self.ap.logger.info(f"Running inactivity check for user {self.target_qq}...")
                try:
                    probability_to_greet = self.current_config.proactive_greeting_probability
                    self.ap.logger.info(
                ...
                self.ap.logger.info(f"Check finished. Sleeping for {self.current_config.loop_time} seconds...")
                try:
                    await asyncio.sleep(self.current_config.loop_time)
                except asyncio.CancelledError:
    ...
    async def _check_user_inactivity(self):  # 检测用户活跃时长
        ...
        last_message_time = self.current_config.memory.get_lastest_time(self.current_config.memory.short_term_memory)  # 时间差值
        ...
        inactivity_threshold_minutes = self.current_config.proactive_min_inactive_hours
        ...
        if inactive_minutes_float > inactivity_threshold_minutes_float:  # 差值大于规定最小时间
            ...
            proactive_do_not_disturb_start = self.current_config.proactive_do_not_disturb_start
            proactive_do_not_disturb_end = self.current_config.proactive_do_not_disturb_end
            time_format = "%H:%M"
    ...
    async def proactive_greeting(self):  ##主动问候生成词
        ...
        if file_exists:
            ...
            raw_prompt = self.current_config.cards.generate_system_prompt()  # 获取角色卡
            full_card_prompt_text = self.current_config.memory.to_custom_names(raw_prompt)
        ...
        conversations = self.current_config.memory.get_normalize_short_term_memory()
        ...
        await self.current_config.memory.save_memory(role="assistant", content=response)  # 主动发言存入到历史记忆当中
